...
async function handleLogin(isQuick) {
  ...
  if (!username || !password)
    return $message.warning('Please enter username and password')
  if (!isQuick && !captcha)
    return $message.warning('Please enter captcha')
  try {
    loading.value = true
  $message.loading('Verifying, please wait...', { key: 'login' })
  ...
  // 10003 is the business code specifically for captcha errors
    if (error?.code === 10003) {
  // To prevent brute-force attacks, refresh the captcha when it's incorrect
      initCaptcha()
    }
